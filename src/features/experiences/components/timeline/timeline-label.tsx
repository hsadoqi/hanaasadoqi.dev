...
const ACTIVE_SLOT_WIDTH = 'w-96 sm:w-[40rem]';
...
const INACTIVE_DOT_CLASS =
  'bg-border/40 group-hover:bg-foreground/60 group-hover:scale-125 motion-safe:transition-all motion-safe:duration-300';
const ACTIVE_LABEL_CLASS = 'text-foreground';
const INACTIVE_LABEL_CLASS =
  'text-muted-foreground/50 group-hover:text-muted-foreground/70 motion-safe:transition-colors motion-safe:duration-300';
const ACTIVE_SUBTITLE_CLASS =
  'text-foreground/90 font-semibold tracking-widest uppercase';
const INACTIVE_SUBTITLE_CLASS = 'text-muted-foreground/40';
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={`Go to ${experience.company} — ${experience.title}. Use arrow keys to navigate or Home/End to jump.`}
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              className={`font-mono text-[9px] leading-snug tracking-wider sm:text-[10px] ${isActive ? 'text-muted-foreground/50' : 'text-muted-foreground/30'}`}
            >
              {experience.period}
            </span>
          </div>
